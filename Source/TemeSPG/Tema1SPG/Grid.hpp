...
public:
	...
	BaseMesh* generateGrid(uint8_t numCells, float cellSize)
	{
		...
		{
			VertexFormat vH1(-extremity, row, 0);
			VertexFormat vH2(extremity - cellSize, row, 0);
//			printf("vH1: %f %f     vH2: %f %f\n", vH1.position.x, vH1.position.y, vH2.position.x, vH2.positiony);
			...
			VertexFormat vV1(row, -extremity, 0);
			VertexFormat vV2(row, extremity - cellSize, 0);
	...
	void DrawGrid()
	{
		...
		glUniform3f(glGetUniformLocation(shaderHandle->GetProgramID(), "color"), 0.1, 0.15, 0.25);
		gridMesh->draw(); //XoY
		model_matrix = glm::rotate(glm::mat4(1), glm::radians(90.f), glm::vec3(1, 0, 0));
		glUniformMatrix4fv(glGetUniformLocation(shaderHandle->GetProgramID(), "model_matrix"), 1, false, glm::value_ptr(model_matrix));
		gridMesh->draw(); //XoZ
